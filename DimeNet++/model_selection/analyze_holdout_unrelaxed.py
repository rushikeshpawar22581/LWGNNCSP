...
if __name__ == '__main__':
    ...
    output_file = "holdout_unrelaxed_metrics_dimenetpp.csv"

    df = pd.DataFrame(columns=["model_type_trial", "metric1", "metric2","total"])

    for i,model_type_trial in enumerate(model_type_trial_list):
        metric1, metric2, total = get_metric1_metric2(f"{path_to_results}/dimenet_pp_{model_type_trial}")
        row = {"model_type_trial": model_type_trial, "metric1": metric1, "metric2": metric2, "total":total}
        df.loc[i] = row
