...
def collate_pool(dataset_list):
    batch_atom_fea, batch_nbr_fea, batch_nbr_fea_idx, batch_adj, batch_edge = [], [], [], [], []
    crystal_atom_idx, batch_targets = [], []
    batch_cif_ids = []
    batch_formula = []
    base_idx = 0
    for i, ((atom_fea, nbr_fea, nbr_fea_idx, adj, edge_tensor), targets, cif_id, formula) in enumerate(dataset_list):
        n_i = atom_fea.shape[0]  # number of atoms for this crystal
        batch_adj.append(adj)
        batch_edge.append(edge_tensor)
...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        adj = adj.flatten()
        # adj = torch.LongTensor(get_molecular_multigraph_adj(nbr_fea_idx))
        # adj = torch.flatten(adj)
        ...
        return (atom_fea, nbr_fea, nbr_fea_idx, adj, edge_tensor), targets, cif_id, crystal
