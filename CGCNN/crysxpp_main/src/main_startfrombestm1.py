...
'''
def save_checkpoint(state, is_best, filename='checkpoint' + args.pid + '.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, '../model/model_best' + args.pid + '.pth.tar')
'''
'''
...
def args_parse():
    ...
    parser.add_argument('--feature-selector', type=str, default='false', help='Option for feature selector')
    parser.add_argument('--pid', type = str, default = None, help = 'PID of run')
...
if args.feature_selector.lower() == 'true':
    args.feature_selector = True
elif args.feature_selector.lower() == 'false':
    args.feature_selector = False
else:
    raise ValueError('Invalid Feature Selector Argument')
...
def main():
    ...
    print(args.pretrained_model != None)
    ...
    if len(dataset) < 2000:
        ...
        sample_data_list = [dataset[i] for i in sample(range(len(dataset)), 2000)]
    ...
    model = CrystalGraphConvNet(orig_atom_fea_len, nbr_fea_len,atom_fea_len=args.atom_fea_len,n_conv=args.n_conv,
                                h_fea_len=args.h_fea_len,n_h=args.n_h,classification=False,feature_selector=args.feature_selector)
    if args.cuda:
        model.cuda()

    path = '../results/Prediction/' + args.pid
    if not os.path.exists(path):
        os.makedirs(path)
    out = open(path + "/out.txt", "w")

    print("Input M1 Model Path " + str(args.pretrained_model))
    out.writelines("Input M1 Model Path " + str(args.pretrained_model))
    out.writelines("\n")
    ...
    ## Parameter Initialization using best M1 model.
    pretrained_path = args.pretrained_model
    if pretrained_path != None:
        model_name = pretrained_path

        if args.cuda:
            m1_model = torch.load(model_name)
        else:
            m1_model = torch.load(model_name, map_location=torch.device('cpu'))

        # Transfer Weights from Encoder of AutoEnc
        '''
        model_dict = m1_model.state_dict()
        ...
        '''
        # Transfer Weights from Best M1 Model
        #m1_model_dict = m1_model.state_dict()
        #new_m1_model_dict = OrderedDict()

        #for key in m1_model_dict.keys():
        #    new_m1_model_dict[key] = m1_model_dict[key]

        #model_dict = model.state_dict()
        #model_dict.update(new_m1_model_dict)
        model.load_state_dict(m1_model['state_dict'])
        normalizer.load_state_dict(m1_model['normalizer'])
    else:
    ...
    val_mae = validate(val_loader, model, criterion, normalizer,args,-1,out)
    print('Validation MAE for epoch is {}'.format(float(val_mae.data.cpu())))
    ...
    for epoch in range(args.epochs):
        ...
        print('Training time per epoch : {}s'.format(round(epoch_end_time - epoch_start_time), 2))
        # evaluate on validation set
        val_mae = validate(val_loader, model, criterion, normalizer,args,epoch,out)

        print('Training Loss for epoch is {}'.format(float(train_loss.data.cpu())))
        print('Training MAE for epoch is {}'.format(float(train_mae.data.cpu())))
        print('Validation MAE for epoch is {}'.format(float(val_mae.data.cpu())))
    ...
    best_checkpoint = torch.load('../model/model_best' + args.pid + '.pth.tar')
    model.load_state_dict(best_checkpoint['state_dict'])
    train_mae=validate(train_loader, model, criterion, normalizer,args,epoch,out)
    #print("Train MAE on Best Validation Model :" + str(train_mae))
    val_mae=validate(val_loader, model, criterion, normalizer,args,epoch,out)
    #print("Validation MAE on Best Validation Model :" + str(val_mae))
    test_mae=validate(test_loader, model, criterion, normalizer,args,epoch,out)
    print('Best validation model obtained after {} epochs'.format(best_epoch))
    print("Train MAE on Best Validation Model :" + str(train_mae))
    print("Validation MAE on Best Validation Model :" + str(val_mae))
