...
def args_parse():
    ...
    parser.add_argument('--data-path', type=str, default='../data_1000/', help='Root Data Path')
    parser.add_argument('--lrate', type=float, default=0.03, help='Learning Rate')
    ...
    parser.add_argument('--pid', type=str, help='Process identifier')
...
def main(seed):
    ...
    path='../results/CrystalAE/'+ 'model_pretrain' + pid
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for input in batch_data_unsup: 
            ...
            input_var = (Variable(atom_fea.to(device)), Variable(nbr_fea.to(device)), nbr_fea_idx.to(device),[crys_idx.to(device) for crys_idx in crys_graph_idx])
            ...
            for j in range(len(edge_prob_list)):
                if global_loss_flag:
                    ...
                    loss = loss + 0.9*loss_adj_reconst
                ...
                if local_loss_flag:
                    ...
                    loss = loss  + 0.1*loss_atom_feat_reconst
        ...
        if local_loss_flag == 1 and global_loss_flag == 1:
            result = "Epoch " + str(epoch + 1) + \
                     ...
                     ", Global (Adjacency) Loss " + str(np.mean(bt_adj_reconst_loss_list)) + \
                     ", Local (Feature) Loss " + str(np.mean(bt_feature_reconst_loss_list))
        ...
        out.writelines("\n")
        if np.mean(bt_loss_list) < min_loss:
            print("Saving a model")
            min_loss = np.mean(bt_loss_list)
            save_model = model
            torch.save(save_model, "../model/model_pretrain" + pid + ".pth")
    plot(train_loss, 'Train Loss Plot', 'Reconstruction Total Loss', 'Number of epoch', path+'/train_loss.png')
    if global_loss_flag:
        plot(train_adj_reconst_loss, 'Global Loss Plot', 'Global Loss', 'Number of epoch',
             path + '/global_loss.png')
    if local_loss_flag:
        plot(train_feature_reconst_loss, 'Local Loss Plot', 'Local Loss', 'Number of epoch',
             path + '/local_loss.png')
