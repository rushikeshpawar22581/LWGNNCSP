...
def get_args():
    argparser = argparse.ArgumentParser(description="Train a SchNet model with KGCNN on given data")
...
def data_generator_xy(dataset, batch_size, hyper):
    '''
    ...
    while True:
        for offset in range(0, len_dataset, batch_size):
            x = dataset[offset:offset+batch_size].tensor(hyper["model"]["config"]["inputs"])
            y = dataset[offset:offset+batch_size].get("graph_labels")
            yield tuple(x),y #tuple(x) is necessary

def data_generator_x(dataset, batch_size, hyper):
    '''
    dataset: CrystalDataset object
    batch_size: int
    '''
    ...
    for offset in range(0, len_dataset, batch_size):
        x = dataset[offset:offset+batch_size].tensor(hyper["model"]["config"]["inputs"])
        yield tuple(x) #tuple(x) is necessary

# Output signature of data
def get_output_signature( ispredict):
    '''
    ispredict: bool whether to yield x only or x and y
    '''
    x_signature = (
    ...
    y_signature = tf.TensorSpec(shape=(None,1), dtype=tf.float32)
    return x_signature, y_signature

def get_dataloader(dataset, batchsize, ispredict, hyper):
    '''
    dataset: CrystalDataset object
    batchsize: int
    ispredict: bool whether to yield x only or x and y
    '''
    output_signature = get_output_signature(ispredict)
    if ispredict:
        return  tf.data.Dataset.from_generator(
                                            generator= lambda : data_generator_x(dataset, batchsize, hyper),
                                            output_signature=output_signature
                                            )
    return  tf.data.Dataset.from_generator(
                                            generator= lambda : data_generator_xy(dataset, batchsize, hyper),
                                            output_signature=output_signature
                                            )
...
def run_train(args, hyper):
    ...
    train_dataloader = get_dataloader(data_train, hyper["training"]["fit"]["batch_size"], ispredict=False, hyper=hyper)
    val_dataloader = get_dataloader(data_val, hyper["training"]["fit"]["batch_size"], ispredict=False, hyper=hyper)
    test_dataloader = get_dataloader(data_test, hyper["training"]["fit"]["batch_size"], ispredict=False, hyper=hyper)

    ########################## create a model ##########################
    schnet_model = deserialize_model(hyper["model"])

    schnet_model.compile(
        optimizer=hyper["training"]["compile"]["optimizer"],
        loss=hyper["training"]["compile"]["loss"],
        metrics=scaled_metrics)

    schnet_model.summary()

    print(" Compiled with jit: %s" % schnet_model._jit_compile)  # noqa
    print(" schnet_model is built: %s, with unbuilt: %s" % (
            all([layer.built for layer in schnet_model._flatten_layers()]),  # noqa
            [layer.name for layer in schnet_model._flatten_layers() if not layer.built]
        ))
    ########################## Start training ##########################
    ...
    start = time.time()
    hist = schnet_model.fit(
        x = train_dataloader,
        epochs=hyper["training"]["fit"]["epochs"],
        steps_per_epoch = len(train_idx)//hyper["training"]["fit"]["batch_size"] + 1,
        validation_data=val_dataloader,
        validation_freq=hyper["training"]["fit"]["validation_freq"],
        validation_steps = len(val_idx)//hyper["training"]["fit"]["batch_size"] + 1,
        verbose=hyper["training"]["fit"]["verbose"],
        callbacks=deserialize_callbacks(hyper["training"]["fit"]["callbacks"])+[checkpoints],
        shuffle=True,
    )
    end = time.time()
    ...
    # save model wights
    schnet_model.save_weights(os.path.join(model_dir, f"last_ep_model_seed_{hyper['info']['random_seed']}.weights.h5"))
    ...
    with open(os.path.join(expt_dir, "hyperparameters.json"), "w") as f:
        json.dump(hyper, f, indent=4)

    logger.info(f"Saved hyperparameters at {expt_dir}/hyperparameters.json")

    ########################## Evaluate model & save plots ##########################
    # load test data
    y_test = np.array(data_test.get("graph_labels"))    # not scaled
    y_val = np.array(data_val.get("graph_labels"))    # scaled

    logger.info("Evaluating last epoch model=======================================================================")
    logger.info("Evaluating on validation set")
    # plot predict true
    y_pred = schnet_model.predict(val_dataloader, steps = len(val_idx)//hyper["training"]["fit"]["batch_size"] + 1)
    y_true = y_val

    plot_predict_true(y_pred, y_true, filepath=result_dir,
                    model_name=f"schnet_last_epoch_{args.model_name}",
    ...
    # plot predict true
    y_pred = schnet_model.predict(test_dataloader, steps = len(test_idx)//hyper["training"]["fit"]["batch_size"] + 1)
    ...
    logger.info("Evaluating on validation set")
    y_pred = best_model.predict(val_dataloader, steps = len(val_idx)//hyper["training"]["fit"]["batch_size"] + 1)
    y_true = y_val
    plot_predict_true(y_pred, y_true, filepath=result_dir,
                    model_name=f"schnet_best_model_{args.model_name}",
                    scaled_predictions=scaled_predictions, target_names=["Formation Energy"],
                    file_name=f"predict_val_seed_{hyper['info']['random_seed']}")

    # plot predict true test for best model
    logger.info("Evaluating on test set")
    y_pred = best_model.predict(test_dataloader, steps = len(test_idx)//hyper["training"]["fit"]["batch_size"] + 1)
    y_pred = scaler.inverse_transform(y_pred)
    y_true = y_test
    plot_predict_true(y_pred, y_true, filepath=result_dir,
                    model_name=f"schnet_best_model_{args.model_name}",
    ...
    plot_train_test_loss([history], model_name=f"schnet_kgcnn",filepath=result_dir,
...
def main():
    ...
    hyper["info"]["postfix_file"] = f"../model/schnet_kgcnn_{model_name}"
