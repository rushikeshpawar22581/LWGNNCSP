...
def write_structs(low, high, scale, num_per, input_dir, output_dir, modelpath, config_file, device):
    ...
    fake_id_prop = pd.DataFrame()
    fake_id_prop['struct_idx'] = perturbed_cif_files
    fake_id_prop['form_energy'] = [0]*len(perturbed_cif_files)
    fake_id_prop.to_csv(os.path.join('./temp_data/', 'id_prop.csv'), header = False, index = False)

    perturbed_predicted_list = predict_energy(modelpath, './temp_data/', config_file, device)
    df_perturbed = pd.DataFrame()
    df_perturbed['struct_idx'] = perturbed_cif_files
    df_perturbed['form_energy'] = perturbed_predicted_list
...
if __name__ == "__main__":
    ...
    output_dir = intermediate_dir + 'modelmappedM1'
    modelpath = '../trained_models/M1_trial3/'
    config_file = '../config_runs/M1_trial3.json'
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    E = write_structs(low, high, scale_per, num_per, intermediate_dir, output_dir, modelpath, config_file, device)
